...
interface UseWhiteboardSocketReturn {
  ...
  emitSendInvite: (
    toUserIds: string[],
    session: { id: string; name: string; creator: SessionUser },
  ) => void;
...
export function useWhiteboardSocket(
  ...
  const { sessionId, user, authToken } = options;

  const [isConnected, setIsConnected] = useState(false);
  const [cursors, setCursors] = useState<Map<string, Cursor>>(new Map());
  const [socketState, setSocketState] = useState<Socket | null>(null);
  const socketRef = useRef<Socket | null>(null);
  const userRef = useRef<SessionUser | undefined>(user);
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      const joinUser = userRef.current;
      if (joinUser) {
        socket.emit("join-session", { user: joinUser });
      }
    });
