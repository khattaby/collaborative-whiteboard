...
export function getElementAtPosition(
  ...
  for (let i = elements.length - 1; i >= 0; i--) {
    ...
    switch (element.type) {
      case "pencil": {
        const threshold = element.size / scale + 5 / scale;
      ...
      case "circle": {
        ...
        const rx = Math.abs(element.width / 2) + 5 / scale;
        const ry = Math.abs(element.height / 2) + 5 / scale;
        const dx = (x - cx) / rx;
        const dy = (y - cy) / ry;
      ...
      case "diamond": {
        ...
        if (
          isPointInDiamond(
            ...
            cx,
            cy,
            Math.abs(element.width / 2) + 5 / scale,
            Math.abs(element.height / 2) + 5 / scale,
          )
        ) {
          return element;
        }
      ...
      case "arrow":
      case "line": {
        // Geometric threshold: size / scale + padding / scale
        const threshold = (element.size || 2) / scale + 5 / scale;
...
export function getElementBounds(
  ...
  switch (element.type) {
    ...
    case "text": {
      const width = element.width || 200; // fallback
      const height = element.height || element.fontSize * 1.5; // fallback
      return {
        minX: element.x,
        minY: element.y,
    ...
    case "sticky": {
      ...
      if (ctx) {
        height = calculateStickyHeight(ctx, element);
      }
      return {
        minX: element.x,
        minY: element.y,
